...
// blob data created using 
// https://github.com/hpcc-systems/GPU-Deep-Learning/blob/master/Datasets/data_files/Dataset%20Preprocessing.ipynb
...
trainX1 := PROJECT(train0, TRANSFORM(
  mnist_data_type_withid_x, SELF.image:=byte2int(LEFT.image), SELF.id := COUNTER, SELF := LEFT));
trainY1 := PROJECT(train0, TRANSFORM(
  mnist_data_type_withid_y, SELF.label:=LEFT.label, SELF.id := COUNTER, SELF:= LEFT));
//output(trainX1);
...
testX1 := PROJECT(test0, TRANSFORM(
  mnist_data_type_withid_x, SELF.image:=byte2int(LEFT.image), SELF.id := COUNTER, SELF := LEFT));
testY1 := PROJECT(test0, TRANSFORM(
  mnist_data_type_withid_y, SELF.label:=LEFT.label, SELF.id := COUNTER, SELF:= LEFT));
