/*
About this test:
  Test the usability of Pre-trained Model NASNetLarge.
  Reference: https://www.tensorflow.org/api_docs/python/tf/keras/applications/nasnet/NASNetLarge
  Input shape = (331, 331, 3) 
  
Results:

class                   probability
tusker	                0.865471601486206
African_elephant	      0.05588202923536301
Indian_elephant	        0.004061948508024216
*/
...
SET OF REAL hexToNparry(DATA byte_array):= EMBED(Python)
  ...
  I_array = tf.keras.applications.nasnet.preprocess_input(I_array)
...
x3 := PROJECT(x2, TRANSFORM(TensData, SELF.indexes := [1, TRUNCATE(LEFT.id/(331*3)) + 1, TRUNCATE(LEFT.id/3)%331 + 1, LEFT.id%3 + 1], SELF.value := LEFT.value));
x := Tensor.R4.MakeTensor([0,331,331,3], x3);

// load the model
s := GNNI.GetSession(1);
ldef := ['''applications.nasnet.NASNetLarge(weights = "imagenet")'''];
